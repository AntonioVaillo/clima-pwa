...
function renderForecast(card, data) {
  ...
  card.querySelector('.current .temperature .value')
      .textContent = Math.round((data.currently.temperature-32)*.5556);
  ...
  futureTiles.forEach((tile, index) => {
    ...
    tile.querySelector('.temp-high .value')
        .textContent = Math.round((forecast.temperatureHigh-32)*.5556);
    tile.querySelector('.temp-low .value')
        .textContent = Math.round((forecast.temperatureLow-32)*.5556);
  });
...
function loadLocationList() {
  ...
  if (!locations || Object.keys(locations).length === 0) {
    const key = '37.6256827,-0.9965839';
    locations = {};
    locations[key] = {label: 'Cartagena', geo: '37.6256827,-0.9965839'};
  }
  return locations;
}
